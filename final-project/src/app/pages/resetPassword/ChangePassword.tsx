...
const ChangePassword = () => {
  ...
  const schema = yup.object().shape({
    newPassword: yup.string().min(8, 'New password must be more than 8 characters').max(20, 'Password must be less than 20 characters').required(),
  });
  const {
    register,
    handleSubmit,
    ...
    <section className="section-change-pass">
      <div className="container">
        ...
        <form className="change-pass-form" onSubmit={handleSubmit(onSubmit)}>
          <input className="password" type="password" placeholder="Old password" {...register("oldPassword")} onChange={(e: FormEvent<HTMLInputElement>) => setPass(e.currentTarget.value)}></input>
          <input className="password" type="password" placeholder="Repeat old password" onChange={(e: FormEvent<HTMLInputElement>) => setRepeatPass(e.currentTarget.value)}></input>
          {errors.repeatPassword ? <span className="error">{errors.repeatPassword.message}</span> : ''}
          <input className="password" type="password" placeholder="New password" {...register("newPassword")}></input>
