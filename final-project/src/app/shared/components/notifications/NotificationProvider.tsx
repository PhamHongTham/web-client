...
const NotificationProvider = ({ children }: any) => {
  ...
  const handleDeleteNotification = (notificationId: string) => {
    setNotifications(notifications => notifications.filter((item: NotificationOptions) => item.id !== notificationId))
  }
  const contextData = {
  ...
  return (
    <NotificationContext.Provider value={contextData}>
      <div className="notification-wrapper">
        {
          notifications.map((notification: NotificationOptions) => <Notification key={notification.id} notification={notification} />)
        }
      </div>
      {
        children
