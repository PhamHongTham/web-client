...
const SignInEmail = ({ handleShowSignInModal, showLoginModal }: SignInEmailPropsOptions) => {
  ...
  useEffect(() => {
    ...
    if (userCurrent) {
      handleAddNotification({ type: 'SUCCESS', message: 'Sign in user success' })
      showLoginModal()
    }
  ...
  return (
    <div className="sign-in-email">
      <h2 className="sign-in-email-title">Join <span className="blog-name">Boogle</span></h2>
      ...
      <Link to="" className="back-all-action" onClick={handleShowSignInModal}><i className="fal fa-chevron-left"></i> All sign in options</Link>
      <p className="sign-in-info">Click “Sign In” to agree to Boogle’s Terms of Service and acknowledge that Boogle’s Privacy Policy applies to you.</p>
      {
        isLoading ? <Loading /> : ''
      }
    </div>
