import { changePassword, getUserInfo, login, signUp, updateUserInfo } from "app/shared/core/services/auth";
...
export const loginRequest = (userInfo: UserLoginOptions) => async (dispatch: any) => {
  ...
  try {
    ...
    localStorage.setItem("USER_ID", JSON.stringify(data.userInfo.id));
  } catch (error: any) {
    dispatch({ type: UserConstant.LOGIN_FAILURE, payload: error.response.data.errors[0] });
  }
};
...
export const signUpRequest = (userInfo: any, callback: () => void) => async (dispatch: any) => {
  ...
  try {
    ...
    dispatch({ type: UserConstant.SIGN_UP_FAILURE, payload: error.response.data.errors[0] });
  }
};
...
export const getUserInfoRequest = () => async (dispatch: any) => {
  ...
  try {
    ...
    dispatch({ type: UserConstant.GET_USER_INFO_FAILURE, payload: error.response.data.errors[0] });
  }
};
...
export const changePasswordRequest = (info: PasswordOptions) => async (dispatch: any) => {
  ...
  try {
    ...
    dispatch({ type: UserConstant.CHANGE_PASSWORD_FAILURE, payload: error.response.data.errors[0] });
  }
};
...
export const updateUserInfoRequest = (info: UserInfoOptions) => async (dispatch: any) => {
  ...
  try {
    ...
    if(idUser){
      ...
      dispatch({ type: UserConstant.UPDATE_USER_INFO_SUCCESS, payload: data });
    }
  } catch (error: any) {
    dispatch({ type: UserConstant.UPDATE_USER_INFO_FAILURE, payload: error.response.data.errors[0] });
  }
};
